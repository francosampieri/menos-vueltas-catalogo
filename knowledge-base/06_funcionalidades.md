...
- Aplicar en la exploración normal del catálogo un orden editorial local, configurable de forma independiente para B2C y B2B por categoría y subcategoría: los `Id_Grupo` priorizados se muestran en el orden exacto definido y los grupos restantes conservan el orden alfabético por nombre y marca. La búsqueda, `Nuevos`, `Descuentos` y la franja de `Nuevos ingresos` conservan sus reglas de orden propias.
...
- Permitir vistas editoriales temporales de eventos B2C, accesibles únicamente desde el CTA de un hero. Cada evento define sus textos, CTA y una lista explícita y ordenada de `Id_Grupo`; muestra sólo grupos activos presentes en el catálogo B2C, sin alterar sus categorías naturales, Nuevos ni el catálogo normal. Sus tarjetas abren el modal habitual y al cerrarlo se vuelve a la misma vista.
