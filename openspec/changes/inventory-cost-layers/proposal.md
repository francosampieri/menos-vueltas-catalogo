...
- Simplificar la tabla principal de Inventario a Producto, Proveedor, Modalidad, Saldo y Valor total. Cada fila abre por click, Enter o Espacio un detalle que explica tandas abiertas, asignaciones de salida, faltantes pendientes, composición e historial de tandas agotadas. En el historial general y en el selector de antecedente de corrección, los UUID quedan sólo como valores internos: las ventas muestran `Pedido #<Id_Pedido>` y las correcciones resuelven su antecedente con una etiqueta humana, fecha corta y cantidad, sin selección manual de tanda ni códigos técnicos visibles.
