...
Un `INGRESO` histórico puede tener esa columna vacía por compatibilidad. Si su modalidad está vacía, o si su costo unitario no alcanza para valorarlo, origina un tramo explícitamente **no valorizable**: conserva cantidad, orden e identidad de tanda, puede participar del saldo físico, pero no entra en capital propio, consignación ni total físico a costo. La UI puede mostrar el valor conocido de los demás tramos, pero debe marcar que el total completo de capital no está disponible; nunca debe presentarlo como capital total. Un ingreso con datos malformados que no sea el caso histórico compatible (por ejemplo, modalidad no vacía fuera del enum, cantidad/costo no numérico o inconsistente, o campos requeridos ausentes en un ingreso nuevo) falla cerradamente para la valorización del producto, sin total parcial engañoso.
...
La tabla principal de Inventario mostrará para cada producto gestionado: producto, modalidad actual informativa, saldo físico existente, capital en stock propio, valor en consignación, valor físico total a costo y un indicador/cantidad de faltante pendiente de costo. Si hay un tramo no valorizable, mostrará los valores conocidos separados y un estado explícito de “capital total incompleto / tramo no valorizable”; no etiquetará el total conocido como capital total completo. Los productos que tengan tandas abiertas continúan visibles con esos valores aunque su modalidad actual sea `CONTRA_PEDIDO`. Sólo lista tandas abiertas en su desglose; una tanda agotada no ocupa la vista principal.
...
Los valores conocidos se calculan con las tandas abiertas valorizables: `capital propio = Σ(remanente × costo)` de `STOCK_PROPIO`; `consignación = Σ(remanente × costo)` de `CONSIGNACION`; `total físico conocido = capital propio + consignación`. Un faltante pendiente o tramo no valorizable no suma valor ni se disfraza como costo cero.

### 6. Compatibilidad y condiciones de fallo

El lector mantiene el saldo actual como suma algebraica de todas las cantidades. Las salidas históricas, los históricos sin snapshot y todo movimiento válido continúan legibles; no se escribe un backfill ni se crean salidas retrospectivas. La excepción de compatibilidad para un `INGRESO` histórico con modalidad vacía o costo insuficiente es un tramo no valorizable, que se omite de los valores conocidos y se muestra como “sin origen de costo histórico”; por eso la respuesta no puede presentar un capital total completo. Cualquier otro incumplimiento del contrato se rechaza cerradamente para la valorización del producto, sin devolver totales parciales como si fueran completos.
...
- [El ledger no conserva hoy una modalidad de ingreso inmutable] → añadir la columna privada sólo para ingresos futuros; tratar el vacío histórico como tramo no valorizable y no usar la modalidad actual para reescribir la historia.
